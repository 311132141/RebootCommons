    ...
    <main class="flex-1 p-6 ">
      ...
      <div class="bg-gray-900 rounded-lg overflow-hidden">
        <div class="p-4 border-b border-gray-800">
          <h2 class="text-lg">전체 기업 현황 및 성과 데이터</h2>
        </div>

        <!-- Table Header -->
        <div class="grid grid-cols-6 gap-4 px-4 py-3 text-sm text-gray-400">
          <div>이름</div>
          <div>담당자 정보</div>
          <div>참여 프로그램</div>
          <div>협업 성장률</div>
        ...
        <div class="space-y-px">
          <div v-for="company in companies" :key="company.id"
            class="grid grid-cols-6 gap-4 px-4 py-3 bg-gray-800 hover:bg-gray-700 transition-colors items-center">
            ...
            <div>{{ company.programs }}</div>
            <div>{{ company.growth }}</div>
            <div>
              <button
                class="px-3 py-1.5 bg-[#1d1e22] rounded-md text-sm hover:bg-[#2c2d31] transition-colors flex items-center space-x-1">
                <span class="material-icons-outlined text-sm">리포트 작성하기</span>
                <!-- <span>리포트 작성하기</span> -->
              </button>
            </div>
            <div>
              <button @click="goToCompanyDashboard(company.id)"
                class="px-3 py-1.5 bg-gray-900 rounded-md text-sm hover:bg-[#2c2d31] transition-colors flex items-center space-x-1">
                <span class="material-icons-outlined text-sm">대시보드 열기</span>
                <!-- <span>대시보드 열기</span> -->
              </button>
            </div>
          </div>
